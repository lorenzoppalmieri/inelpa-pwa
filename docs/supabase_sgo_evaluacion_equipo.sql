-- Evaluación compartida SGO, sin transferir el seguimiento ni habilitar el cierre ajeno.
begin;
...
declare
  ...
  campos_evaluacion constant text[] := array['beneficioEsperado', 'prioridad', 'presupuestoEstimado', 'fechaObjetivo', 'decision', 'decisionEn', 'decisionPor', 'fechaRevision', 'justificacionDecision'];
...
begin
  ...
  if tg_op = 'INSERT' or old.mejora is null then
    if decision_nueva <> 'pendiente' and actor = any(equipo_sgo) then
  ...
  if (
    new.mejora
      - (case when actor = any(equipo_sgo) then campos_evaluacion else array[]::text[] end)
      - 'gestorSGO'
      - 'verificacionSGOEn'
      - 'verificacionSGOPor'
  ...
  if decision_nueva is distinct from decision_anterior then
    if not (actor = any(equipo_sgo)) then
      raise exception 'Solo Lorenzo, Lara, Nicolás o Azul pueden registrar la decisión.';
    end if;
    new.mejora := jsonb_set(new.mejora, '{decisionEn}', to_jsonb(now()::text), true);
    new.mejora := jsonb_set(new.mejora, '{decisionPor}', to_jsonb(actor), true);
  end if;
  ...
  if new.estado is distinct from old.estado then
    ...
    elsif actor <> gestor_nuevo and not (actor = any(equipo_sgo) and new.estado in ('en_analisis', 'con_acciones')) then
