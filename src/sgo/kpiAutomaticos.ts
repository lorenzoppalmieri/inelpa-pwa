...
function periodosHasta(periodo: string, cantidad = 6): string[] {
  const [anio, mes] = periodo.split('-').map(Number)
  if (!anio || !mes) return [periodo]
  return Array.from({ length: cantidad }, (_, indice) => {
    const fecha = new Date(Date.UTC(anio, mes - 1 - (cantidad - 1 - indice), 1))
...
export function detalleKPIAutomatico(indicador: IndicadorSGO, d: DatosKPIAutomaticos): DetalleKPIAutomatico {
  ...
  const evolucion = periodosHasta(periodo).map((p) => ({ periodo: p, valor: clave ? calcular(clave, area, p, d) : undefined }))
  if (indicador.origen !== 'automatico' || !clave) {
    return { formula: 'Valor ingresado manualmente por el responsable del indicador.', registros: [], evolucion: [{ periodo, valor: indicador.valorActual }] }
  }
  const hoy = new Date().toISOString()
  if (clave === 'produccion_cumplimiento') {
    const base = d.tareas.filter((t) => SECTOR_A_AREA[t.sectorId] === area && t.tipo !== 'reparacion' && enPeriodo(t.inicioPlanificado, periodo) && (t.inicioPlanificado ?? '') <= hoy)
  ...
  if (clave === 'produccion_retrabajos') {
    const tareasArea = d.tareas.filter((t) => SECTOR_A_AREA[t.sectorId] === area)
    const reparaciones: RegistroDetalleKPI[] = tareasArea.filter((t) => t.tipo === 'reparacion' && enPeriodo(t.finReal ?? t.inicioReal ?? t.creada, periodo)).map((t) => ({
    ...
    const paradas: RegistroDetalleKPI[] = tareasArea.flatMap((t) => t.paradas.filter((p) => enPeriodo(p.inicio, periodo) && CAUSAS_PARADA.find((c) => c.id === p.causa)?.categoria === 'calidad').map((p) => ({
