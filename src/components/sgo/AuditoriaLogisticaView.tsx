...
import {
  PLANTILLAS_AUDITORIA_LOGISTICA, auditoriaCompleta, calcularCumplimientoAuditoria, calcularCubicaje,
  checklistDesdePlantilla, puntosNoConformes,
  type DatosAuditoriaLogistica, type EspacioCarga, type EstadoPuntoAuditoria, type ItemCarga, type TipoAuditoriaLogistica,
} from '../../sgo/logistica'
...
function CalculadoraCubicaje() {
  ...
  const [items, setItems] = useState<ItemCarga[]>([
    { id: crypto.randomUUID(), descripcion: 'Transformador', cantidad: 1, largoM: 2, anchoM: 1.4, altoM: 2.1, pesoKg: 3500, rotatable: false, apilable: false, color: COLORES_CARGA[0] },
  ])
  ...
  function setItem(id: string, campo: keyof ItemCarga, valor: string | number | boolean) {
    setItems((actuales) => actuales.map((i) => i.id === id ? { ...i, [campo]: valor } : i))
  }
  function agregar() {
    setItems((actuales) => [...actuales, { id: crypto.randomUUID(), descripcion: '', cantidad: 1, largoM: 1, anchoM: 1, altoM: 1, pesoKg: 100, rotatable: true, apilable: false, color: COLORES_CARGA[actuales.length % COLORES_CARGA.length] }])
  }
  function descargarCSV() {
    const lineas = [
      ['Descripción', 'Cantidad', 'Largo m', 'Ancho m', 'Alto m', 'Peso unitario kg', 'Rotable', 'Apilable'],
      ...items.map((i) => [i.descripcion, i.cantidad, i.largoM, i.anchoM, i.altoM, i.pesoKg, i.rotatable ? 'Sí' : 'No', i.apilable ? 'Sí' : 'No']),
      [],
  ...
  return <div>
    ...
    <div className="sgo-carga-items">
      {items.map((i, index) => <div className="card sgo-carga-item" key={i.id} style={{ borderLeftColor: i.color }}>
        ...
        {([['cantidad', 'Cantidad', 1], ['largoM', 'Largo m', .01], ['anchoM', 'Ancho m', .01], ['altoM', 'Alto m', .01], ['pesoKg', 'kg/unidad', .1]] as [keyof ItemCarga, string, number][]).map(([campo, label, step]) => <div className="field" key={campo}><label>{label}</label><input className="input" type="number" min={step} step={step} value={i[campo] as number} onChange={(e) => setItem(i.id, campo, Math.max(step, Number(e.target.value) || 0))} /></div>)}
    ...
    <div className="sgo-carga-layout">
      <div>
        <div className="card-header"><div className="section-title" style={{ margin: 0 }}>Vista superior orientativa</div><button className="btn" onClick={descargarCSV}>Exportar CSV</button></div>
        <div className="sgo-carga-plano" style={{ aspectRatio: `${Math.max(.1, espacio.largoM)} / ${Math.max(.1, espacio.anchoM)}` }}>
          {resultado.ubicaciones.map((p) => <div className="sgo-carga-bulto" key={p.id} title={`${p.descripcion} · ${p.unidades} unidad(es)`} style={{ left: `${(p.xM / espacio.largoM) * 100}%`, top: `${(p.yM / espacio.anchoM) * 100}%`, width: `${(p.largoM / espacio.largoM) * 100}%`, height: `${(p.anchoM / espacio.anchoM) * 100}%`, background: p.color }}><span>{p.descripcion}</span>{p.unidades > 1 && <b>×{p.unidades}</b>}</div>)}
        </div>
        <div className="meta">Frente del camión a la izquierda · cálculo conservador por filas y apilado de bultos iguales.</div>
      </div>
      <div className="card sgo-carga-advertencias"><strong>Verificaciones obligatorias</strong>{resultado.advertencias.map((a) => <div key={a}>• {a}</div>)}</div>
    </div>
